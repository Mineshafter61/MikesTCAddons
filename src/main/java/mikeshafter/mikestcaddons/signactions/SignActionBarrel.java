...
public class SignActionBarrel extends SignActionStation {
  
  @Override
  public boolean match(SignActionEvent info) {
    return info.isType("barrelsta", "specialsta", "barrelrun", "specialrun") && info.getMode() != SignActionMode.NONE;
  }
  ...
  public void barrel(SignActionEvent info, MinecartGroup group) {
      ...
      try {
        ...
        for (String key : data.keySet()) {
          ...
          if (group.getProperties().getTags().containsAll(tags) && data.get(key) instanceof Map dataMap) {
            ...
            plugin.getLogger().warning("Barrel's barrel book is in the wrong format!");
        }
    ...
    if (info.isType("barrelsta", "specialsta"))
      super.execute(info);
  }
